...
export class PatternEditor {
    ...
    private _renderedThirds: boolean[] = [false, false];
    ...
    private _snapToPitch(guess: number, min: number, max: number): number {
        ...
        const scale: ReadonlyArray<boolean> = this._doc.prefs.notesOutsideScale ? createScales(this._doc.song.edo).dictionary["Free"].flags : createScales(this._doc.song.edo)[this._doc.song.scale].flags;
        ...
        if (this._mouseDragging && this._mouseDown && this._cursor.valid && continuousState) {
            ...
            if (this._draggingStartOfSelection) {
                ...
                if (this._mouseDragging && pattern != null) {
                    ...
                    const notesInScale: number = createScales(this._doc.song.edo)[this._doc.song.scale].flags.filter(x => x).length;
    ...
    public render(): void {
        ...
        if ((this._renderedThirds[0] != this._doc.prefs.showThird) || (this._renderedThirds[1] != this._doc.prefs.showJustThird)) {
            this._renderedThirds = [this._doc.prefs.showThird, this._doc.prefs.showJustThird];
